...
func registerGeneralAsset(db StateDB, accountAddr common.Address, nonce uint64, desc string) (common.Address, error) {
	...
	assetAddress := crypto.CreateAssetAddress(accountAddr, nonce, info.Name)
	...
	setAccountList(AccountModel, db, info.Owner, assetAddress)
	key := info.Owner.String() + assetAddress.String()
	b = new(bytes.Buffer)
...
func issueGeneralAsset(db StateDB, ownerAddr common.Address, assetAddr common.Address, value *big.Int) error {
	//get asset balance
	var info AccountAssetInfo
...
func enoughGeneralBalance(db StateDB, targetAddr common.Address, assetAddr common.Address, value *big.Int) (bool, error) {
	key := targetAddr.String() + assetAddr.String()
	selfAsset := db.GetAccount(targetAddr, key)
	var balance *big.Int
