...
func TestStateChangeDuringTransactionPoolReset(t *testing.T) {
	...
	var (
		...
		assetdb    = asset.NewAsset(statedb)

		trigger = false
	)

	// setup pool with 2 transaction in it
	assetdb.AddBalance(address, types.ZipAssetID, new(big.Int).SetUint64(params.Ziper))
...
func TestInvalidTransactions(t *testing.T) {
	...
	pool.currentAsset.AddBalance(from, types.ZipAssetID, big.NewInt(1))
	if err := pool.AddRemote(tx); err != ErrInsufficientFunds {
		t.Error("expected", ErrInsufficientFunds)
	}

	pool.currentAsset.AddBalance(from, types.ZipAssetID, tx.Cost())
	if err := pool.AddRemote(tx); err != ErrIntrinsicGas {
		t.Error("expected", ErrIntrinsicGas, "got", err)
	}

	pool.currentAsset.SetNonce(from, 1)
	pool.currentAsset.AddBalance(from, types.ZipAssetID, big.NewInt(0xffffffffffffff))
...
func TestTransactionQueue(t *testing.T) {
	...
	tx := transaction(0, 100, key)
	from, _ := deriveSender(tx)
	pool.currentAsset.AddBalance(from, types.ZipAssetID, big.NewInt(1000))
	pool.lockedReset(nil, nil)
	...
	from, _ = deriveSender(tx)
	pool.currentAsset.SetNonce(from, 2)
	pool.enqueueTx(tx.Hash(), tx)
	pool.promoteExecutables([]common.Address{from})
...
func TestTransactionNegativeValue(t *testing.T) {
	...
	tx, _ := types.SignTx(newTx(0, big.NewInt(-1), 100, big.NewInt(1), nil), types.NewSigner(params.DefaultChainconfig.ChainID), key)
...
func TestTransactionChainFork(t *testing.T) {
	...
	resetAsset := func() {
		statedb, _ := state.New(common.Hash{}, state.NewDatabase(zdb.NewMemDatabase()))
		assetdb := asset.NewAsset(statedb)
		assetdb.AddBalance(addr, types.ZipAssetID, big.NewInt(100000000000000))
...
func TestTransactionDoubleNonce(t *testing.T) {
	...
	tx1, _ := types.SignTx(newTx(0, big.NewInt(100), 100000, big.NewInt(1), nil), signer, key)
	tx2, _ := types.SignTx(newTx(0, big.NewInt(100), 1000000, big.NewInt(2), nil), signer, key)
	tx3, _ := types.SignTx(newTx(0, big.NewInt(100), 1000000, big.NewInt(1), nil), signer, key)
...
func TestTransactionMissingNonce(t *testing.T) {
	...
	addr := crypto.PubkeyToAddress(key.PublicKey)
	pool.currentAsset.AddBalance(addr, types.ZipAssetID, big.NewInt(100000000000000))
	tx := transaction(1, 100000, key)
	if _, err := pool.add(tx, false); err != nil {
		t.Error("didn't expect error", err)
	}
...
func TestTransactionNonceRecovery(t *testing.T) {
	...
	addr := crypto.PubkeyToAddress(key.PublicKey)
	pool.currentAsset.SetNonce(addr, n)
	pool.currentAsset.AddBalance(addr, types.ZipAssetID, big.NewInt(100000000000000))
	pool.lockedReset(nil, nil)
	...
	pool.currentAsset.SetNonce(addr, n-1)
	pool.lockedReset(nil, nil)
	if fn := pool.pendingAsset.GetNonce(addr); fn != n-1 {
...
func TestTransactionDropping(t *testing.T) {
	...
	pool.currentAsset.AddBalance(account, types.ZipAssetID, big.NewInt(1000))
	...
	pool.currentAsset.AddBalance(account, types.ZipAssetID, big.NewInt(-650))
	pool.lockedReset(nil, nil)
...
func TestTransactionPostponing(t *testing.T) {
	...
	for i := 0; i < len(keys); i++ {
		...
		pool.currentAsset.AddBalance(crypto.PubkeyToAddress(keys[i].PublicKey), types.ZipAssetID, big.NewInt(50100))
	}
	...
	for _, addr := range accs {
		pool.currentAsset.AddBalance(addr, types.ZipAssetID, big.NewInt(-1))
	}
	pool.lockedReset(nil, nil)
...
func TestTransactionGapFilling(t *testing.T) {
	...
	account, _ := deriveSender(transaction(0, 0, key))
	pool.currentAsset.AddBalance(account, types.ZipAssetID, big.NewInt(1000000))
...
func testTransactionQueueGlobalLimiting(t *testing.T, nolocals bool) {
	...
	for i := 0; i < len(keys); i++ {
		keys[i], _ = crypto.GenerateKey()
		pool.currentAsset.AddBalance(crypto.PubkeyToAddress(keys[i].PublicKey), types.ZipAssetID, big.NewInt(1000000))
	}
...
func testTransactionQueueTimeLimiting(t *testing.T, nolocals bool) {
	...
	pool.currentAsset.AddBalance(crypto.PubkeyToAddress(local.PublicKey), types.ZipAssetID, big.NewInt(1000000000))
	pool.currentAsset.AddBalance(crypto.PubkeyToAddress(remote.PublicKey), types.ZipAssetID, big.NewInt(1000000000))
...
func testTransactionLimitingEquivalency(t *testing.T, origin uint64) {
	...
	pool1.currentAsset.AddBalance(account1, types.ZipAssetID, big.NewInt(1000000))
	...
	pool2.currentAsset.AddBalance(account2, types.ZipAssetID, big.NewInt(1000000))
...
func TestTransactionCapClearsFromAll(t *testing.T) {
	...
	key, _ := crypto.GenerateKey()
	addr := crypto.PubkeyToAddress(key.PublicKey)
	pool.currentAsset.AddBalance(addr, types.ZipAssetID, big.NewInt(1000000))
...
func TestTransactionPoolRepricingKeepsLocals(t *testing.T) {
	...
	for i := 0; i < len(keys); i++ {
		keys[i], _ = crypto.GenerateKey()
		pool.currentAsset.AddBalance(crypto.PubkeyToAddress(keys[i].PublicKey), types.ZipAssetID, big.NewInt(1000*1000000))
	}
...
func TestTransactionReplacement(t *testing.T) {
	...
	key, _ := crypto.GenerateKey()
	pool.currentAsset.AddBalance(crypto.PubkeyToAddress(key.PublicKey), types.ZipAssetID, big.NewInt(1000000000))
...
func testTransactionJournaling(t *testing.T, nolocals bool) {
	...
	pool.Stop()
	assetdb.SetNonce(crypto.PubkeyToAddress(local.PublicKey), 1)
	blockchain = &testBlockChain{statedb, 1000000, new(feed.Feed)}
	...
	assetdb.SetNonce(crypto.PubkeyToAddress(local.PublicKey), 2)
	pool.lockedReset(nil, nil)
